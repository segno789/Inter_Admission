...
class Privateslips extends CI_Controller {
    ...
    */
    ...
    public function languagesslip()
    {
        $this->load->helper('url');
        $data = array(
            'message' => '',
        );
        $this->load->view('Privateslips/langslip.php',$data);
    ...
    private function makepdf12class($pdf,$info)
    {
      ...
      //    $info['slips'][$tprcount]['prcount'] = 3;
           ...
           $Y = $Y + 1;
            ...
            { 
                ...
                else
                {
                    ...
                    // DebugBreak();
                    if($lablen>55 && $lablen<110)
                    {
                        if($issecond == 0 and $isthird ==0 and ($l ==2 || $l ==1))
                            $Y = $Y + 5.0;
                        else
                        {
                            $Y = $Y + 10.0; 
        ...
        $pdf->SetFont('Arial','',8);
        $pdf->Image("assets/img/CE_Signature.png",170.0,257, 30,30, "PNG");  
        $pdf->SetXY(157,288);
    ...
    public function GetlangpvtRSlip()
    {
        //DebugBreak()  ;
        $this->load->helper('url');
        $sess=mSession1;
        ...
        $this->load->model('RollNoSlip_model');
        $studeninfo = array('data'=>$this->RollNoSlip_model->getPVTLangthrslip($stdName,$Fname,$Fnic,$rno,$FormNo,$class,$year,$sess));
        if($studeninfo['data'] == 0)
        {
            $datamsg = array(
                'message' => 'Record Not Found',
            );  
            $this->load->view('Privateslips/languagesslip.php',$datamsg);
        }
        else
        {
            $rno = $studeninfo['data']['info'][0]['Rno'];
            ...
            if(!$isexists)
            {
                ...
                $pdf->AddPage();
                //$html = $this->load->view('Privateslips/MatricRollNo', $studeninfo['data']['info'][0], true);   
                $this->makepdflangthclass($pdf, $studeninfo['data']['info'][0]);
                // $pdf->writeHTML($html, true, false, true, false, '');  
                $pdf->Output($rno.'.pdf', 'I');  
            }
    ...
    private function makepdf11thclass($pdf,$info)
    {
      ...
      // else
        ...
        $pdf->Image("assets/img/note_11th.jpg",10.0,262, 145,23, "JPG"); 
        $pdf->Image("assets/img/CE_Signature.png",163.0,252, 32,32, "PNG"); 
    ...
    private function makepdflangthclass($pdf,$info)
    {

        $info['Rno'] = '000'.$info['Rno'];
        if($info['Session'] ==1) $Session= 'ANNUAL'; else $Session='SUPPLY';
        if($info['errmessage'] == null)  $errmessage = '(PROVISIONALLY)'; else{ $errmessage = ' (PROVISIONALLY OBJECTION SLIP)';};
      // else

        if($info['grp_cd'] == 1)  $grp_cd = 'FAZAL ARABIC';
        else if($info['grp_cd'] == 2) $grp_cd='FAZAL URDU';
            else if($info['grp_cd'] == 3) $grp_cd='FAZAL PUNJABI';
                    else if($info['grp_cd'] == 5) $grp_cd='ADEEB ARABIC';
                        else if($info['grp_cd'] == 6) $grp_cd='ADEEB URDU';
                            else if($info['grp_cd'] == 7) $grp_cd='ADEEB PUNJABI';
                                else if($info['grp_cd'] == 8) $grp_cd='ADEEB PERSIAN';
                                    else if($info['grp_cd'] == 9) $grp_cd='ALIM ARABIC';
                                        else if($info['grp_cd'] == 10) $grp_cd='ALIM URDU';
                                            else if($info['grp_cd'] == 11) $grp_cd='FAZAL PERSIAN';
                                            else if($info['grp_cd'] == 12) $grp_cd='ALIM PERSIAN';
                                                if($info['Gender']==1) $Gender= 'MALE'; else if($info['Gender']==2) $Gender= 'FEMALE';
             $filepath = DIRPATH12TH.$info['picpath'];
           //$filepath = 'assets/img/download.jpg';
        ...
        $pdf->SetFont('Arial','',9);
        $pdf->SetXY(26.2,10.9);
        $pdf->Cell(0, 0.2, "ROLL NUMBER SLIP (WITH DATE SHEET) FOR LANGUAGES $Session EXAMINATION, ".$info["Year"], 0.25, "C");  
 ...
 $pdf->SetFillColor(255,255,255);
            ...
            for($l = 0; $l<$info['slips'][$countter]['subp1count']; $l++) { 
                if($info['slips'][$l+$countter]['class'] == 11) {
                    ...
                    if($info['slips'][$l+$countter]['sub_Name'] == 1)
                    {
                        $info['slips'][$l+$countter]['sub_Name'] = 'Paper-I';
                    }
                    else if($info['slips'][$l+$countter]['sub_Name'] == 2)
                    {
                        $info['slips'][$l+$countter]['sub_Name'] = 'Paper-II';
                    }
                    else if($info['slips'][$l+$countter]['sub_Name'] == 3)
                    {
                        $info['slips'][$l+$countter]['sub_Name'] = 'Paper-III';
                    }
                     else if($info['slips'][$l+$countter]['sub_Name'] == 4)
                    {
                        $info['slips'][$l+$countter]['sub_Name'] = 'Paper-IV';
                    }
                     else if($info['slips'][$l+$countter]['sub_Name'] == 5)
                    {
                        $info['slips'][$l+$countter]['sub_Name'] = 'Paper-V';
                    }
                      else if($info['slips'][$l+$countter]['sub_Name'] == 6)
                    {
                        $info['slips'][$l+$countter]['sub_Name'] = 'Paper-VI';
                    }
                    $Y = $Y + $cellheight;
        ...
        //  DebugBreak();
        if(@$info['slips'][$countter]['subp1count'] <3)
        {
              $Y = 22+ $Y;
        }
