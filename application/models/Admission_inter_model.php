...
class Admission_inter_model extends CI_Model 
{
    ...
    public function Delete_NewEnrolement($formno)
    {
        ...
        $this->db->update('Admission_online..IAAdm',$data);
        return true;
    ...
    public function GetFormNo($Inst_Id)
    {
        ...
        $formno = $this->db->get_where('Admission_online..IAAdm', array('coll_cd' => $Inst_Id));
    ...
    public function GetFormNo_pvt()
    {
        ...
        $this->db->select('formno');
        $this->db->order_by("formno", "DESC");
        $formno = $this->db->get_where('Admission_online..IAAdm', array('regPvt' => 2));
        $rowcount = $formno->num_rows();
    ...
    public function user_info($User_info_data)
    {
        ...
        if($rowcount > 0)
        {
            ...
            if($spl_cd == "0")
            {
                $q1         = $this->db->query("select * from Admission_online..IAAdm where coll_cd =$Inst_cd and (isdeleted = 0 or isdeleted is null) and (batch_id = 0 or batch_id is null) and RegGrp =$RegGrp");
            }
            else{
